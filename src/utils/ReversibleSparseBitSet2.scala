...
case class ReversibleSparseBitSet2(val context: ReversibleContext, val n: Int, val initialValues: Iterable[Int]) {
  ...
  class BitSet(values: Iterable[Int]) {
    ...
    //override def clone(): ReversibleSparseBitSet2
    //this.clone()
  }
  ...
  var nWords = bitLength(n)

  var words: Array[Long] = Array.fill(nWords)(0L)
  private[this] val lastMagics = Array.fill(nWords)(-1L)

  var nonZeroIdx: Array[Int] = Array.tabulate(nWords)(i => i)
  var nNonZero: Int = nWords
  ...
  //foreach(v => setBit(words, v))

  /*initialValues.foreach(v => print(v + ", "))
  println
  complete.foreach(v => print(v + ", "))
  println*/
   ...
   */
   def clear(): Unit = {
   words = Array.fill(nWords)(0xFFFFFFFFFFFFFFFFL)
   nNonZero = n/64 + 1 
  ...
  def filter(sets: Array[BitSet], itemIdx: Array[Int], limit: Int, withoutIdx: Int, max: Int): Boolean = {
    /*
    if (nNonZero == 0) {
      println("here")
      return false
    ...
    while (i > 0) {
      ...
      while (j > 0) {
        ...
        val idx = itemIdx(j)
        if (idx != withoutIdx) { 
          myWord &= sets(itemIdx(j)).words(offset) 
          //sets = sets & sets(itemIdx(j)).words(offset)
        }
      }
      ...
      /*if (java.lang.Long.bitCount(myWord) >= max) { 
        return true
      }*/
    }
  ...
  /*def copy(): ReversibleSparseBitSet2
    new ReversibleSparseBitSet2()*/
